    ...
    const columns = useMemo<ColumnDef<AdminOrder>[]>(() => [
        {
            ...
            header: '№ Заказа',
            cell: ({ row }) => <span className="font-mono font-semibold text-yellow-400">#{row.getValue('orderNumber')}</span>,
        },
        {
            ...
            <div className="p-6">
                ...
                <div className="flex items-center justify-between mt-6 text-sm text-zinc-400">
                    ...
                    <div className="flex gap-3">
                        <button onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className="px-4 py-2 border border-zinc-700 rounded-lg disabled:opacity-50">
                            Назад
                        </button>
                        <button onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className="px-4 py-2 border border-zinc-700 rounded-lg disabled:opacity-50">
                            Вперед
                        </button>
                    </div>
                </div>
